...
export default function FullDeal({trade,result,risk,documents,controls,finance,policy,voyage,sourcing,approvals,members,milestones,onNavigate}) {
  ...
  const rows=[
    ['Trade',`${trade.product} · ${Number(trade.volumeMt||0).toLocaleString('en-US')} MT`,`${trade.incoterm} · ${trade.route}`],
    ['Economics',`${result.netMarginPct.toFixed(2)}% net margin`,`$${Math.round(result.netProfit).toLocaleString('en-US')} projected net profit`],
    ...
    ['Access',`${members.filter(item=>item.status==='active').length} active · ${members.filter(item=>item.status==='pending').length} pending`,'Role and margin scopes are enforced by the API'],
  ];
  return <section className="full-deal-panel" id="full-deal">
    ...
    <h3>Open a section</h3><div className="full-deal-links">{sections.map(([label,id])=><button key={id} onClick={()=>onNavigate(id,id)}>{label}<span aria-hidden="true">→</span></button>)}</div>
      <p className="full-deal-note">This summary reflects the signed-in user’s authorised view. Pending participants have no deal access until they accept a matching email invitation.</p>
  </section>;
}
