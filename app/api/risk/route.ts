...
export async function POST(request: Request) {
  ...
  if (body.action === "create_case") {
    ...
    await env.DB.prepare(`INSERT INTO risk_cases (id, owner_id, trade_reference, case_type, title, severity, status, amount_cents, counterparty, occurred_at, description, resolution, updated_at, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).bind(id, user.userId, reference, body.caseType, text(body.title, 140), validSeverities.includes(body.severity) ? body.severity : "medium", "open", cents(body.amount), text(body.counterparty, 120), body.occurredAt ? Date.parse(body.occurredAt) : now, text(body.description, 1200), "", now, now).run();
  ...
  if (body.action === "update_case") {
    ...
    const result = await env.DB.prepare(`UPDATE risk_cases SET status = ?, resolution = ?, updated_at = ? WHERE id = ? AND owner_id = ? AND trade_reference = ?`).bind(body.status, text(body.resolution, 1200), now, id, user.userId, reference).run();
