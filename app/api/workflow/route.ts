...
export async function GET(request: Request) {
  ...
  if (!access) return respond({ error: "Workflow access denied" }, 403);
  const row = await env.DB.prepare(`SELECT current_stage AS currentStage, milestones_json AS milestonesJson, target_delivery AS targetDelivery, updated_at AS updatedAt FROM trade_workflows WHERE owner_id = ? AND trade_reference = ? ORDER BY updated_at DESC LIMIT 1`).bind(access.ownerId, reference).first<any>();
...
export async function POST(request: Request) {
  ...
  if (!access) return respond({ error: "Workflow edit permission required" }, 403);
  const currentStage = Math.max(0, Math.min(7, Number(body.currentStage) || 0));
  const now = Date.now();
  const existing = await env.DB.prepare(`SELECT id, created_at FROM trade_workflows WHERE owner_id = ? AND trade_reference = ? ORDER BY updated_at DESC LIMIT 1`).bind(access.ownerId, reference).first<any>();
