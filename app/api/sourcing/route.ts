...
export async function GET(request: Request) {
  ...
  const row = await env.DB.prepare(`SELECT id, ranking_mode AS rankingMode, selected_quote_key AS selectedQuoteKey, quotes_json AS quotesJson, status, updated_at AS updatedAt FROM sourcing_comparisons WHERE owner_id = ? AND trade_reference = ? ORDER BY updated_at DESC LIMIT 1`).bind(user.userId, reference).first<any>();
...
export async function POST(request: Request) {
  ...
  const now = Date.now();
  const existing = await env.DB.prepare(`SELECT id, created_at AS createdAt FROM sourcing_comparisons WHERE owner_id = ? AND trade_reference = ? ORDER BY updated_at DESC LIMIT 1`).bind(user.userId, reference).first<any>();
  const id = existing?.id ?? crypto.randomUUID();
  await env.DB.prepare(`INSERT INTO sourcing_comparisons (id, owner_id, trade_reference, ranking_mode, selected_quote_key, quotes_json, status, updated_at, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?) ON CONFLICT(id) DO UPDATE SET ranking_mode=excluded.ranking_mode, selected_quote_key=excluded.selected_quote_key, quotes_json=excluded.quotes_json, status=excluded.status, updated_at=excluded.updated_at`).bind(id, user.userId, reference, ["margin","risk","funding"].includes(body.rankingMode) ? body.rankingMode : "margin", clean(body.selectedQuoteKey, 40), JSON.stringify(quotes), ["draft","review","selected","approved"].includes(body.status) ? body.status : "draft", now, existing?.createdAt ?? now).run();
