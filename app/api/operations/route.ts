...
export async function GET(request: Request) {
  ...
  const [counterparties, control] = await Promise.all([
    env.DB.prepare(`SELECT id, name, type, country, status, risk_rating AS riskRating FROM counterparties WHERE owner_id = ? ORDER BY name LIMIT 100`).bind(user.userId).all(),
    reference ? env.DB.prepare(`SELECT counterparty_id AS counterpartyId, controls_json AS controlsJson, approval_status AS approvalStatus, updated_at AS updatedAt FROM trade_controls WHERE owner_id = ? AND trade_reference = ? ORDER BY updated_at DESC LIMIT 1`).bind(user.userId, reference).first<any>() : null,
  ]);
...
export async function POST(request: Request) {
  ...
  if (body.action === "save_controls") {
    ...
    const existing = await env.DB.prepare(`SELECT id, created_at FROM trade_controls WHERE owner_id = ? AND trade_reference = ? ORDER BY updated_at DESC LIMIT 1`).bind(user.userId, reference).first<any>();
