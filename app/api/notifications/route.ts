...
export async function GET(request: Request) {
  ...
  return respond(await list(user.userId, reference));
}
...
export async function POST(request: Request) {
  ...
  if (body.action === "sync") {
    ...
    const existingResult = await env.DB.prepare(`SELECT id, event_key AS eventKey, status FROM notification_events WHERE owner_id = ? AND trade_reference = ?`).bind(user.userId, reference).all<any>();
    ...
    for (const event of incoming) {
      ...
      if (match) statements.push(env.DB.prepare(`UPDATE notification_events SET category=?, severity=?, title=?, message=?, target=?, status='active', last_seen_at=? WHERE id=? AND owner_id=?`).bind(event.category,event.severity,event.title,event.message,event.target,now,match.id,user.userId));
      else statements.push(env.DB.prepare(`INSERT INTO notification_events (id, owner_id, trade_reference, event_key, category, severity, title, message, target, status, read_at, last_seen_at, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'active', NULL, ?, ?)`).bind(crypto.randomUUID(),user.userId,reference,event.eventKey,event.category,event.severity,event.title,event.message,event.target,now,now));
    }
    for (const item of existing) if (item.status === "active" && !activeKeys.has(item.eventKey)) statements.push(env.DB.prepare(`UPDATE notification_events SET status='resolved', last_seen_at=? WHERE id=? AND owner_id=?`).bind(now,item.id,user.userId));
    if (statements.length) await env.DB.batch(statements);
  } else if (body.action === "mark_read") {
    await env.DB.prepare(`UPDATE notification_events SET read_at = ? WHERE id = ? AND owner_id = ? AND trade_reference = ?`).bind(now,clean(body.id,80),user.userId,reference).run();
  } else if (body.action === "mark_unread") {
    await env.DB.prepare(`UPDATE notification_events SET read_at = NULL WHERE id = ? AND owner_id = ? AND trade_reference = ?`).bind(clean(body.id,80),user.userId,reference).run();
  } else if (body.action === "mark_all_read") {
    await env.DB.prepare(`UPDATE notification_events SET read_at = ? WHERE owner_id = ? AND trade_reference = ? AND status = 'active' AND read_at IS NULL`).bind(now,user.userId,reference).run();
